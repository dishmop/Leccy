...
public class SetupResolution : MonoBehaviour {
	...
	void ReduceResolution(){
		...
		if (newResIndex != resIndex){
			...
			GoogleAnalytics.Client.SendTimedEventHit("reduceResolution",  LevelManager.singleton.GetCurrentLevelName(), (resolutions[newResIndex].width + "x" + resolutions[newResIndex].height), Time.fixedTime);
